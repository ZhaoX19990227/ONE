...
Page({
  data: {
    ...
    customBrandName: '', customItemName: '', amountYuan: '', rating: 0,
  ...
  inputBrand(event: WechatMiniprogram.Input) { this.setData({ customBrandName: event.detail.value, brandId: 0 }) },
  inputItem(event: WechatMiniprogram.Input) { this.setData({ customItemName: event.detail.value, itemId: 0 }) },
  ...
  async save() {
    ...
    const amountFen = this.data.amountYuan ? Math.round(Number(this.data.amountYuan) * 100) : undefined
