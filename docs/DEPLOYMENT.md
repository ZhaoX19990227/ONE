...
```bash
cp .env.example .env
# 填写域名、微信、数据库、DashScope 和 DeepSeek 参数
...
百炼工作空间的兼容地址按地域填写，例如北京地域：

```dotenv
QWEN_BASE_URL=https://<workspace-id>.cn-beijing.maas.aliyuncs.com/compatible-mode/v1
```

## DeepSeek

DeepSeek 只增强“记忆推荐”，普通转盘不调用模型。生产环境通过 `.env` 配置：
...
DEEPSEEK_TIMEOUT=PT6S
```

不要把真实密钥写入 `.env.example`、Compose 文件或 Git。调用只包含候选名称、当前时段和匿名口味理由；失败会回退到本地记忆引擎。指标 `one_recommendation_ai_total` 可区分 `success/invalid/fallback`。
...
- DeepSeek 模型名会调整；上线前确认控制台可用模型，并只通过 `DEEPSEEK_MODEL` 切换，不改代码。
